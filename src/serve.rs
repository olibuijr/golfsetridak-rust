...
pub fn run(cfg: Config) -> io::Result<()> {
    ...
    server.run_with(middleware, move |req: &Request| {
        Reply::Response(dispatch(&root, &store, &shop_store, &cart_store, req))
    })
}
...
fn dispatch(
    ...
    match path {
        ...
        "/api/admin/shop/products" => api_admin_products(shop_store, req, None),
        "/api/admin/shop/categories" => api_admin_categories(shop_store, req, None),
        "/api/cart" => api_cart(cart_store, req),
        "/api/cart/items" => api_cart_items(store, shop_store, cart_store, req, None),
        "/api/cart/items/bulk" => api_cart_items_bulk(store, shop_store, cart_store, req),
        p if p.starts_with("/api/admin/shop/products/") => api_admin_products(
            shop_store,
            req,
            Some(slug_of(p, "/api/admin/shop/products/")),
        ),
        p if p.starts_with("/api/admin/shop/categories/") => api_admin_categories(
            shop_store,
            req,
        ...
        "/um-okkur" => about(root),
        "/verslun" => shop_page(root, shop_store),
        "/my/verslun" => shop_page(root, shop_store),
        "/my/verslun/karfa" => cart_page(root, cart_store, req),
        "/admin/vorur" => admin_products_page(root, shop_store),
        "/admin/vorur/flokkar" => admin_categories_page(root, shop_store),
        "/admin/vorur/nytt" => admin_product_form_page(root, shop_store, None),
        p if p.starts_with("/admin/vorur/") => {
            admin_product_form_page(root, shop_store, Some(slug_of(p, "/admin/vorur/")))
        }
...
fn api_availability(store: &BookingStore, req: &Request) -> Response {
...
fn api_slot_price(store: &BookingStore, req: &Request) -> Response {
    if req.method != Method::Get {
        return json(405, &error_value("method not allowed"));
    }
    let q = query_pairs(req);
...
fn api_book(store: &BookingStore, req: &Request) -> Response {
...
fn api_bookings(store: &BookingStore, req: &Request) -> Response {
    let q = query_pairs(req);
    match req.method {
        Method::Get => {
...
fn api_cart(store: &CartStore, req: &Request) -> Response {
    if req.method != Method::Get {
        return json(405, &error_value("method not allowed"));
    }
    let cookie = cart::cookie_cart_id(req);
    match store.get_or_create_open(cookie.as_deref(), now_ms()) {
        Ok((summary, created)) => cart_json(200, &summary, created),
...
fn api_cart_items(
    ...
    let cookie = cart::cookie_cart_id(req);
    let (summary, created) = match cart_store.get_or_create_open(cookie.as_deref(), now_ms()) {
        Ok(v) => v,
    ...
    match req.method {
        Method::Post => {
            ...
            match cart_store.add_item(&summary.id, item, now_ms()) {
                Ok(next) => cart_json(201, &next, created),
                Err(e) => json(400, &error_value(&e)),
            }
        }
        Method::Put | Method::Patch => {
            ...
            match cart_store.update_quantity(&summary.id, id, quantity, now_ms()) {
                Ok(next) => cart_json(200, &next, created),
...
fn api_cart_items_bulk(
    ...
    let cookie = cart::cookie_cart_id(req);
    let (mut summary, created) = match cart_store.get_or_create_open(cookie.as_deref(), now_ms()) {
        Ok(v) => v,
        Err(e) => return json(500, &error_value(&e)),
    };
    for raw in items {
...
fn shop_page(root: &Path, store: &ShopStore) -> Response {
    render(
        root,
        "shop",
        vec![
...
fn cart_page(root: &Path, store: &CartStore, req: &Request) -> Response {
    let cookie = cart::cookie_cart_id(req);
    match store.get_or_create_open(cookie.as_deref(), now_ms()) {
        Ok((summary, created)) => {
            let mut resp = render(
...
fn admin_products_page(root: &Path, store: &ShopStore) -> Response {
    render(
        root,
        "admin_products",
        vec![
...
fn admin_categories_page(root: &Path, store: &ShopStore) -> Response {
    render(
        root,
        "admin_categories",
        vec![
...
fn admin_product_form_page(root: &Path, store: &ShopStore, id: Option<&str>) -> Response {
    let product = match id {
        Some(id) => match store.product(id) {
            Some(product) => Some(product),
            None => return not_found_page(root),
        },
        None => None,
    };
...
fn calendar_page(root: &Path, store: &BookingStore) -> Response {
    let now = now_ms();
